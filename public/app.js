document.addEventListener('DOMContentLoaded', () => {
  ...
  // ELEMENTOS DA SEÇÃO FECART (AVALIAÇÃO DE SEGURANÇA REAL)
  ...
  // Enter para navegar ou submeter
  if (userNameInput) {
  ...
  // Toggle visualização da senha na tela de diagnóstico
  ...
  // Resumo explicativo da força da senha
  ...
  // Atualiza um critério individual (pílula)
  ...
  // Popula os resultados da avaliação no card da FECART
  ...
  // ==========================================
  // SUBMETER VERIFICAÇÃO E EXIBIR AVALIAÇÃO REAL
  // ==========================================
  async function submitCheck() {
    ...
    try {
      ...
      // Atualiza mensagem de boas-vindas com o nome do participante se fornecido
      if (fecartWelcomeMsg) {
        if (userName && userName !== 'Visitante') {
          fecartWelcomeMsg.textContent = `Olá, ${userName}! Obrigado pela sua participação na demonstração prática de cibersegurança!`;
        } else {
          fecartWelcomeMsg.textContent = 'Obrigado pela sua participação na demonstração prática de cibersegurança!';
        }
      }

      // Popula os resultados da avaliação no card
      populateFecartPasswordEvaluation();

      // Transição: oculta formulário de entrada e exibe avaliação real
